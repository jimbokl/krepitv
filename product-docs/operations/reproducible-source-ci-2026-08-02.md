...
CI устанавливает интерфейс строго через `web/package-lock.json`, запускает
единый `npm run build`, а затем требует нулевой tracked diff. Таким образом,
изменение исходников без соответствующего проверенного production artifact не
проходит gate. Публикация остаётся отдельной операцией Pages и не входит в CI.
