...
- [ ] **Step 3: Реализовать семь result panels**
  ...
  Совместимость, VESA-винты/проставки, крепление к стене, персональная карта высот и геометрия, кабели, инструменты/расходники, упорядоченный чек-лист. Не выводить пустые числовые поля как нули.

- [ ] **Step 4: Реализовать печать**

  Кнопка вызывает `window.print()`. В `@media print` скрыть навигацию и CTA, раскрыть чек-лист, сохранить названия модели/кронштейна, статусы, предупреждения и источники. Всегда показывать доступные отметки персональной карты высот; reference line пластины — только при подтверждённых смещениях. Для полностью доказанной геометрии дополнительно добавить масштабную SVG-карту отверстий с контрольной линейкой 100 мм; для остальных сценариев — без точек сверления. Не добавлять отдельный PDF-движок в первый релиз.
