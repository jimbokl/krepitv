...
- [ ] **Step 1: Написать падающие result-тесты**
  ...
  Проверить наличие семи секций, явные статусы, отсутствие unsafe exact-claim, отсутствие коммерческого CTA у `blocked`, сохранение полезного чек-листа без оффера и полный порядок элементов печати.
...
- [ ] **Step 4: Реализовать печать**
  ...
  Кнопка вызывает `window.print()`. В `@media print` скрыть навигацию и CTA, раскрыть чек-лист, сохранить названия модели/кронштейна, статусы, предупреждения и источники.
