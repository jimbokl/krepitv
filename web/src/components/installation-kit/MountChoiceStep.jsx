...
function MountOption({ match, selected, onChange }) {
  return (
    <label className={`flex cursor-pointer flex-col border-2 bg-white p-4 focus-within:ring-2 focus-within:ring-action ${selected ? "border-action" : "border-ink"}`}>
      <input checked={selected} className="sr-only" name="mount-id" onChange={() => onChange(match.mount.id)} type="radio" />
