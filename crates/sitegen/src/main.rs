...
const CORE_PAGES_UPDATED_AT: &str = "2026-08-20";
...
fn editorial_accountability_html(content_kind: &str, checked_at: &str) -> String {
    ...
    format!(
        "<aside aria-label=\"Как подготовлен и проверен материал\" class=\"border-b border-line py-4\" data-editorial-accountability=\"true\"><div class=\"flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-muted\"><span class=\"inline-flex items-center gap-2\">{user_icon} Материал подготовил <a class=\"font-semibold text-ink underline underline-offset-4\" href=\"{author_path}\">{author_name}</a></span><span>Обновлено <time datetime=\"{checked_at}\">{visible_date}</time></span></div><details class=\"group mt-2 max-w-4xl\"><summary class=\"flex min-h-11 cursor-pointer list-none items-center gap-2 font-semibold text-technical focus:outline-none focus-visible:ring-2 focus-visible:ring-action\">Как мы проверили сведения<span aria-hidden=\"true\" class=\"text-xl text-action transition group-open:rotate-45\">+</span></summary><div class=\"grid gap-3 rounded-lg border border-line bg-white p-4 text-sm leading-relaxed text-muted lg:grid-cols-2\"><p>{shield_icon} На чём основан материал: {basis}</p><p>{check_icon} Дата проверки: <time datetime=\"{checked_at}\">{visible_date}</time></p><p>{info_icon} Испытание товара: {physical_test_label}</p><p>{source_policy}</p><p>{automation_disclosure}</p><p>{physical_test_explanation}</p><p>Подробности: <a class=\"font-semibold text-technical underline underline-offset-4\" href=\"{methodology_path}\">методика</a> · <a class=\"font-semibold text-technical underline underline-offset-4\" href=\"{corrections_path}\">сообщить об ошибке</a>.</p></div></details></aside>",
    ...
    let (name, description, identifier, version, download_base, keywords, variables, files) =
        match page_id {
            "vesa" => (
                ...
                "Проверяемая таблица из 161 точной модели телевизора: полный код модели, размер VESA по горизонтали и вертикали, дата проверки и источник паспорта.",
...
fn commercial_profile_html(profile: &CommercialProfile) -> String {
    ...
    let faq = profile
        ...
        .map(|item| {
            format!(
                "<details class=\"border-t border-line first:border-t-0\"><summary class=\"flex min-h-14 cursor-pointer list-none items-center justify-between gap-4 py-3 font-display text-base font-bold marker:content-none\"><span>{question}</span><span aria-hidden=\"true\" class=\"font-mono text-xl leading-none\">+</span></summary><p class=\"max-w-3xl pb-4 pr-8 text-sm leading-relaxed text-muted sm:text-base\">{answer}</p></details>",
    ...
    format!(
        "<section aria-label=\"Проверка подбора\" class=\"border-b border-line py-5\" data-commercial-profile=\"{kind}:{id}\"><details class=\"group\"><summary class=\"flex min-h-12 cursor-pointer list-none items-center justify-between gap-4 font-display text-lg font-bold marker:content-none\"><span>Как мы проверили подбор</span><span aria-hidden=\"true\" class=\"text-action group-open:rotate-45\">+</span></summary><div class=\"grid gap-6 pb-4 pt-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(22rem,1.1fr)] lg:gap-10\"><div><p class=\"font-mono text-xs uppercase tracking-[0.12em] text-action\">{kicker}</p><h2 class=\"mt-3 max-w-3xl font-display text-2xl font-extrabold leading-tight sm:text-3xl\" id=\"{heading_id}\">{heading}</h2><p class=\"mt-4 max-w-3xl text-base leading-relaxed text-muted\">{answer}</p></div><div aria-label=\"Частые вопросы\" class=\"border-y border-ink\">{faq}</div></div></details></section>",
...
fn model_page_body(
    ...
    let limitations = if tv.limitations.is_empty() {
        ...
        let items = tv
            ...
            .join("");
        format!(
            "<div class=\"mt-5 border-l-2 border-action pl-4\"><h3 class=\"font-display text-xl font-bold\">Границы паспортных данных</h3><ul class=\"mt-3 list-disc space-y-2 pl-5 text-sm leading-relaxed text-muted\">{items}</ul></div>"
        )
    };
    let source_evidence = format!(
        "<section class=\"border-t border-line py-8\" data-source-evidence=\"true\"><p class=\"font-mono text-xs uppercase text-action\">Паспортная проверка</p><h2 class=\"mt-2 font-display text-3xl font-extrabold\">Что подтверждено источником</h2><p class=\"mt-4 max-w-4xl text-lg leading-relaxed\">{source_fact}</p>{limitations}<a class=\"mt-5 inline-flex font-semibold text-technical underline underline-offset-4\" href=\"{source}\" rel=\"noreferrer\">Источник характеристик: {source_label}</a></section>",
        source_fact = escape_html(&tv.source_fact),
    ...
    let installation_kit_cta = format!(
        "<section class=\"border-y-2 border-ink py-7\" data-model-installation-kit=\"{}\"><p class=\"font-mono text-xs uppercase text-action\">Следующий практический шаг</p><h2 class=\"mt-2 font-display text-3xl font-extrabold\">Соберите монтажный комплект для этой модели</h2><p class=\"mt-3 max-w-3xl leading-relaxed text-muted\">Модель уже выбрана. Останется указать стену, механизм, кронштейн, размещение и подключения.</p><a class=\"primary-button mt-5\" href=\"/podbor/?model={}\">Продолжить с {} →</a></section>",
        escape_html(&tv.id),
        escape_html(&tv.id),
        escape_html(&tv.title),
    );
    ...
    static_layout(&format!(
        ...
        installation_kit_cta = installation_kit_cta,
...
fn seo_calculator_note(page_id: &str) -> &'static str {
    match page_id {
        "vesa-200x100" => {
            r#"<section class="border-y-2 border-ink py-7" data-vesa-size-reference="200x100"><h2 class="font-display text-3xl font-bold">Проверка VESA 200×100</h2><p class="mt-3">Между центрами отверстий телевизора должно быть 200 мм по горизонтали и 100 мм по вертикали. Это 20×10 см, а не размер стеновой площадки.</p><div class="mt-5 overflow-x-auto"><table class="w-full text-left"><caption class="sr-only">Условия совместимости VESA 200×100</caption><thead><tr><th class="p-3">Проверка</th><th class="p-3">Что должно совпасть</th></tr></thead><tbody><tr><th class="p-3">Отверстия ТВ</th><td class="p-3">200×100 мм, в этом порядке</td></tr><tr><th class="p-3">Направляющие кронштейна</th><td class="p-3">Явная поддержка 200×100, а не только максимальный размер</td></tr><tr><th class="p-3">После VESA</th><td class="p-3">Масса, диагональ, винты и зазор разъёмов по паспортам</td></tr></tbody></table></div><p class="mt-4">Ниже — точные модели из паспортного каталога и проверенные пары. Интерактивная сверка списка VESA доступна после загрузки страницы.</p><p class="mt-3"><a class="underline" href="https://www.vesa.org/wp-content/uploads/2010/12/FDMI-Overview.pdf" rel="noreferrer" target="_blank">VESA: назначение интерфейса крепления дисплеев</a></p></section>"#
        }
        "vesa-200x300" => {
            r#"<section class="border-y-2 border-ink py-7" data-vesa-size-reference="200x300"><h2 class="font-display text-3xl font-bold">Проверка VESA 200×300</h2><p class="mt-3">Между центрами отверстий телевизора: 200 мм по горизонтали и 300 мм по вертикали, или 20×30 см. Не меняйте числа местами: 300×200 — другая схема.</p><div class="mt-5 overflow-x-auto"><table class="w-full text-left"><caption class="sr-only">Условия совместимости VESA 200×300</caption><thead><tr><th class="p-3">Проверка</th><th class="p-3">Что должно совпасть</th></tr></thead><tbody><tr><th class="p-3">Отверстия ТВ</th><td class="p-3">200×300 мм, в этом порядке</td></tr><tr><th class="p-3">Направляющие кронштейна</th><td class="p-3">Явная поддержка 200×300; 300×200 недостаточно</td></tr><tr><th class="p-3">После VESA</th><td class="p-3">Масса, диагональ, винты и зазор разъёмов по паспортам</td></tr></tbody></table></div><p class="mt-4">Точная пара VESA не задаёт точки сверления стены. Ниже — паспортные модели и проверенные пары.</p><p class="mt-3"><a class="underline" href="https://www.vesa.org/wp-content/uploads/2010/12/FDMI-Overview.pdf" rel="noreferrer" target="_blank">VESA: назначение интерфейса крепления дисплеев</a></p></section>"#
        }
        "vesa-400x300" => {
            r#"<section class="border-y-2 border-ink py-7" data-vesa-size-reference="400x300"><h2 class="font-display text-3xl font-bold">Проверка VESA 400×300</h2><p class="mt-3">Между центрами отверстий телевизора: 400 мм по горизонтали и 300 мм по вертикали, или 40×30 см. Квадрат 400×400 не подтверждает эту пару автоматически.</p><div class="mt-5 overflow-x-auto"><table class="w-full text-left"><caption class="sr-only">Условия совместимости VESA 400×300</caption><thead><tr><th class="p-3">Проверка</th><th class="p-3">Что должно совпасть</th></tr></thead><tbody><tr><th class="p-3">Отверстия ТВ</th><td class="p-3">400×300 мм, в этом порядке</td></tr><tr><th class="p-3">Направляющие кронштейна</th><td class="p-3">Явная поддержка 400×300, не только 400×400</td></tr><tr><th class="p-3">После VESA</th><td class="p-3">Масса, диагональ, винты и зазор разъёмов по паспортам</td></tr></tbody></table></div><p class="mt-4">Точная пара VESA не задаёт точки сверления стены. Ниже — паспортные модели и проверенные пары.</p><p class="mt-3"><a class="underline" href="https://www.vesa.org/wp-content/uploads/2010/12/FDMI-Overview.pdf" rel="noreferrer" target="_blank">VESA: назначение интерфейса крепления дисплеев</a></p></section>"#
        }
...
fn seo_mechanism_catalog_html(
    ...
    let selection = match page.kind.as_str() {
        ...
        "slim" => (
            "Низкопрофильные кронштейны с паспортными размерами".to_string(),
            "В таблице только модели с указанным производителем минимальным отступом не более 35 мм. Это исходный фильтр, а не обещание, что телевизор с кабелями поместится: учитывайте выступ штекера, толщину направляющих, форму задней панели и вентиляцию.".to_string(),
            mounts
                .iter()
                .filter(|mount| {
                    mount.wall_distance_min_mm <= 35.0
    ...
    selected.sort_by(|left, right| {
        if page.kind == "slim" {
            left.wall_distance_min_mm
                .total_cmp(&right.wall_distance_min_mm)
                .then_with(|| left.title.to_lowercase().cmp(&right.title.to_lowercase()))
        } else {
            left.brand
                .to_lowercase()
    ...
    } else if page.kind == "slim" {
        let table_rows = selected
            .iter()
            .map(|mount| format!(
                "<tr class=\"border-t border-line align-top\"><th class=\"p-3 text-left\" scope=\"row\"><a class=\"font-semibold text-action underline\" href=\"/kronshteyny/{id}/\">{title}</a></th><td class=\"p-3\">{distance} мм</td><td class=\"p-3\">{mechanism}</td><td class=\"p-3\">{count}</td><td class=\"p-3\"><a class=\"text-technical underline\" href=\"{source}\" rel=\"noreferrer\" target=\"_blank\">Паспорт</a></td></tr>",
                id = escape_html(&mount.id),
                title = escape_html(&mount.title),
                distance = format_mm(mount.wall_distance_min_mm),
                mechanism = escape_html(mechanism_label(&mount.mechanism)),
                count = verified_model_count(&mount.id, graph),
                source = escape_html(&mount.source_url),
            ))
            .collect::<Vec<_>>()
            .join("\n");
        format!(
            "<div class=\"overflow-x-auto\"><table class=\"w-full min-w-[680px] text-left\" data-slim-mount-table=\"true\"><caption class=\"p-3 text-left font-display text-xl font-bold\">Паспортные отступы и проверенные модели</caption><thead><tr><th class=\"p-3\">Кронштейн</th><th class=\"p-3\">Мин. отступ</th><th class=\"p-3\">Механизм</th><th class=\"p-3\">Проверенных ТВ</th><th class=\"p-3\">Источник</th></tr></thead><tbody>{table_rows}</tbody></table></div>"
        )
    } else if page.kind == "mount-brand" {
...
fn main() {
    ...
    fs::copy(
        data.join("wall_fixing_systems.json"),
        public_data.join("wall-fixing-systems.json"),
    )
    .expect("Не удалось скопировать справочник настенного крепежа");
    fs::copy(
        data.join("model_ports.json"),
        public_data.join("model-ports.json"),
    )
    .expect("Не удалось скопировать паспорта разъёмов");
    fs::copy(
        data.join("connection_profiles.json"),
        public_data.join("connection-profiles.json"),
    )
    .expect("Не удалось скопировать профили подключений");
    fs::copy(
    ...
    write(
        ...
        &html_shell(
            "Монтажный комплект для телевизора по модели — KREPI TV",
            "Выберите модель телевизора, стену и кронштейн. Получите проверку VESA, карту высот, крепёжные ограничения, кабели и чек-лист монтажа.",
...
mod tests {
    ...
    fn editorial_accountability_is_prerendered_for_all_content_families() {
        ...
        for body in bodies {
            ...
            assert!(body.contains("class=\"group mt-2 max-w-4xl"));
    ...
    fn matcher_starts_with_brand_then_model_without_phantom_selection() {
        ...
        assert!(html.contains("Как мы проверяем совместимость"));
        assert!(html.contains("Что нельзя определить без осмотра"));
        assert!(html.contains("Персональная карта высот"));
        assert!(html.contains("VESA не является схемой отверстий в стене"));
    ...
    #[test]
    fn verified_model_pages_link_to_the_single_installation_kit_canonical() {
        let models: Vec<TvModel> = read_json(&workspace_root().join("data/tv_models.json"));
        ...
        for tv in &models {
            ...
            assert!(!html.contains("data-model-matches-jump=\"true\""));
        }
    }
    ...
    #[test]
    fn demand_backed_model_cohort_prerenders_source_fact_and_limitations() {
        let root = workspace_root();
        let models: Vec<TvModel> = read_json(&root.join("data/tv_models.json"));
        let mounts: Vec<Mount> = read_json(&root.join("data/mounts.json"));
        let seo_pages: Vec<SeoPage> = read_json(&root.join("data/seo_pages.json"));
        let tv = models
            .iter()
            .find(|tv| tv.id == "tuvio-td65ufbhh12")
            .expect("Нет demand-backed модели Tuvio TD65UFBHH12");
        let matches = model_mount_matches(tv, &mounts);
        let body = model_page_body(tv, &matches, &[], 0, &seo_pages, None);

        assert!(body.contains("Официальная карточка Tuvio подтверждает точную модель TD65UFBHH12"));
        assert!(body.contains("Модельный год производителем не указан"));
        assert!(body.contains("Резьба и длина винтов настенного крепления не указаны"));
        assert!(body.contains("href=\"https://tuvio.yandex.ru/card/"));
    }
