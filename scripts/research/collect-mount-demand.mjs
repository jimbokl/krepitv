...
const SOURCE_LABEL = "Яндекс Wordstat через XMLRiver: запрос в кавычках и квадратных скобках, Россия, все устройства";
...
const seedPath = path.resolve(
  ROOT,
  argument("--seeds", "data/research/mount-demand-seeds.json"),
);
const outputRoot = path.resolve(
  ROOT,
  argument("--output", `product-docs/research/raw/wordstat-mount-models-${date}`),
);
const manifestOutput = path.resolve(
  ROOT,
  argument("--manifest-output", "data/research/mount-model-demand.json"),
);
...
for (let index = 0; index < seeds.length; index += 1) {
  ...
  const seoFrequency = readFrequency(response, seed.query);
  ...
  process.stdout.write(`${index + 1}/${seeds.length} ${seed.query}: ${seoFrequency}\n`);
  await new Promise((resolve) => setTimeout(resolve, 500));
}

normalized.sort((left, right) =>
  right.seo_frequency - left.seo_frequency || left.query.localeCompare(right.query, "ru"));
const batchSha256 = createHash("sha256")
  .update(JSON.stringify(seeds.map(({ brand, model, query }) => ({ brand, model, query }))))
  .digest("hex");
const manifest = {
  schema_version: 1,
  research_contract: {
    product: "KREPI TV",
    landing_page: "https://krepitv.ru/kronshteyny/",
    intended_conversion: "Проверка совместимости и переход к точной карточке кронштейна на Яндекс Маркете",
    relevance_rule: "Точные бренд и модель кронштейна; без объединения похожих артикулов и цветовых версий",
    region_id: Number(REGION_ID),
    region: "Россия",
    period: "последние 30 дней на дату сбора",
    devices: "все устройства",
    scope: "поисковый спрос Wordstat; не CPC, не прогноз кликов и не награда",
...
const columns = [
  "demand_rank", "brand", "model", "query", "catalog_id", "reward_ui_rank",
  "operator_query", "seo_frequency", "seo_region_id", "seo_region", "seo_period",
  "seo_devices", "observed_at", "source_url",
];
const csv = [
  columns.join(","),
  ...normalized.map((row, index) =>
    columns.map((column) => csvCell(column === "demand_rank" ? index + 1 : row[column])).join(",")),
].join("\n");
